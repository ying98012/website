---
title: FlowStock｜智慧雙軌 AI 進銷存
slug: flowstock
summary: 面向中小商家的 Local-First Flutter 進銷存 App：訂單驅動庫存流水、多量詞成本與端雲雙軌 AI 經營顧問。
techStack:
  - Flutter
  - Bloc
  - " Isar"
  - RevenueCat
  - Gemini
  - Cloudflare Workers
  - Firebase Analytics
coverImage: /uploads/flowstock/Frame 811.png
coverAspect: square
screenshots:
  - /uploads/flowstock/Frame 804.png
  - /uploads/flowstock/Frame 812.png
  - /uploads/flowstock/Frame 813.png
  - /uploads/flowstock/Frame 814.png
  - /uploads/flowstock/Frame 815.png
  - /uploads/flowstock/Frame 816.png
  - /uploads/flowstock/Frame 817.png
videoUrl: https://youtu.be/6EX1-liT6_M?si=1AQI5Sxt6vm9moub
readmeUrls:
  - label: README 展示頁
    url: https://ying98012.github.io/portfolio-readmes/Flowstock%E5%BA%AB%E5%AD%98%E7%AE%A1%E7%90%86/
  - label: play 商店
    url: https://play.google.com/store/apps/details?id=com.flowstock.ainventory&pcampaignid=web_share
featured: true
publishedAt: 2026-07-25
---
為中小商家與自由創業者打造的輕量進銷存系統：本機優先、訂單驅動異動、多量詞換算與加權平均成本，並以端側／雲端雙軌 AI 產出經營建議。

- 進貨當下可建檔、待收貨／待出庫確認後才動庫存；流水帳可追溯與反向作廢
- 多量詞換算（箱／瓶等）與基準量詞加權平均成本；儀表板今日營收／毛利／進貨支出可下鑽明細
- 雙軌 AI 顧問（端側 Gemini Nano → 雲端／Proxy）+ RevenueCat 訂閱閘門；核心進銷存免費（30 品項上限）

